...
- Clarified the job output checkboxes and API documentation so `keep_japanese` promises `.ja.srt`, while `write_ass` separately adds `.en.ass` and adds `.ja.ass` only when both options are enabled.
- Expanded the in-app Setup curl example to explicitly enable `debug_mode`, `keep_japanese`, and `write_ass`, with inline explanations covering detailed per-segment diagnostics and retained Japanese `.ja.srt`/`.ja.ass` sidecars.
